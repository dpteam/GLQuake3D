...
//#if	!id386 // D_SpriteDrawSpans explicitly moved from assembly to C to avoid
//		  crashes, e.g. running timedemo bigass1 in 1280x1024 fullscreen
...
=====================
*/
static void ChkBounds (byte **ptr, byte *start, int size, char *object, sspan_t *pspan)
{
	if (*ptr < start || *ptr - start > size)
	{
		Con_Printf ("\002D_SpriteDrawSpans: ");
		Con_Printf ("%s error (%d, s=%d,u=%d,v=%d)\n", object, *ptr - start, size, pspan->u, pspan->v);
		*ptr = start;
	}
}
...
// we count on FP exceptions being turned off to avoid range problems
	...
	do
	{
		...
		ChkBounds (&pdest, d_viewbuffer, d_viewsize, "pdest", pspan);
		ChkBounds (&(byte *)pz, (byte *)d_pzbuffer, d_pzsize, "pz", pspan);
	...
	do
	{
		...
		// adjust u to ceil the integer portion
			...
			for (v=itop ; v<ibottom ; v++)
			{
				...
				if (pspan->u < 0)
					pspan->u = 0; // This might happen with disastrous results if not fixed

				pspan->v = v;
				
				if (pspan->v < 0)
					pspan->v = 0; // This might happen with disastrous results if not fixed
