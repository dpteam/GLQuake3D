...
void D_RasterizeAliasPolySmooth (void)
{
	...
	if (initialleftheight >= MAXHEIGHT || initialrightheight >= MAXHEIGHT)
		Sys_Error ("D_RasterizeAliasPolySmooth1: invalid a_spans index (%d, max = %d)", initialleftheight >= MAXHEIGHT ? initialleftheight : initialrightheight, MAXHEIGHT);
...
// scan out the bottom part of the right edge, if it exists
	if (pedgetable->numrightedges == 2)
	{
		...
		if (initialrightheight + height >= MAXHEIGHT)
			Sys_Error ("D_RasterizeAliasPolySmooth2: invalid a_spans index (%d, max = %d)", initialrightheight + height, MAXHEIGHT);
