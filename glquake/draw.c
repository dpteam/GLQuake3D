...
qpic_t	*Draw_PicFromWad (char *name)
{
	...
	// Sanity ...
	if (pic->width & 0xC0000000 || pic->height & 0xC0000000)
		Sys_Error ("Draw_PicFromWad: invalid dimensions (%dx%d) for '%s'", pic->width, pic->height, name);
...
void Draw_Pic (int x, int y, qpic_t *pic)
{
	...
	{
		Sys_Error ("Draw_Pic: bad coordinates (%d, %d)", x, y);
	}
...
void Draw_CharToConback (int num, byte *dest, int width)
{
	int		row, col;
	byte	*source;
	int		drawline;
...
//	char		ver[100];
	...
	if (conback->width > 320 && conback->width != lastwidth)
	{
		lastwidth = conback->width; // Prevent multiple warnings
		Con_Printf ("\002Draw_ConsoleBackground: ");
		Con_Printf ("excessive conback width %d (normal max = %d)\n", conback->width, 320);
	}
...
// hack the version number directly into the pic
	...
	dest = conback->data + conback->width*(conback->height - 14) + conback->width - 11 - 8*strlen(ver);
#elif defined(X11)
	...
	for (x=0 ; x<strlen(ver) ; x++)
		Draw_CharToConback (ver[x], dest+(x<<3), conback->width);
#endif
