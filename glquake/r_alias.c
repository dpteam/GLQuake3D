...
================
*/
int R_ChkFrame (char *function, aliashdr_t *pahdr, float *plastmsg)
{
	...
	if (frame >= pmdl->numframes || frame < 0)
	{
		    ...
		    frame > currententity->model->maxframe)
		{
			...
			// Single frame?
			if (pahdr->frames[0].type == ALIAS_SINGLE)
				Con_DPrintf ("%d ('%s', %d frames)", frame, pahdr->frames[0].name, pmdl->numframes);
			else
				Con_DPrintf ("group %d (%d groups)", frame, pmdl->numframes);
...
// construct the base bounding box for this frame
	frame = R_ChkFrame ("R_AliasCheckBBox", pahdr, &lastmsg);
...
void R_AliasSetupSkin (void)
{
	...
	if (skinnum >= pmdl->numskins || skinnum < 0)
	{
		if (IsTimeout (&lastmsg, 2))
			Con_DPrintf ("R_AliasSetupSkin: no such skin %d (%d skins) in %s\n", skinnum, pmdl->numskins, currententity->model->name);
...
void R_AliasSetupFrame (void)
{
	...
	frame = R_ChkFrame ("R_AliasSetupFrame", paliashdr, &lastmsg);
